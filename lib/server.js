...
const clearModuleCache = require('./clear-module-cache')
const checkIfExistModule = require('./check-if-exist-module')
...
module.exports = ({ port, baseDir, apiMapFile, httpsOpts }) => {
  ...
  let baseDirWatcher = fs.watch(baseDir, {
    ...
    if (filename) {
      clearModuleCache(path.join(baseDir, filename), baseDir)
    }
  })
  ...
  app.use((req, res) => {
    ...
    try {
      ...
      Promise.resolve(_module).then(data => {
        res.json(data)
      ...
      if (e.code === 'MODULE_NOT_FOUND' && !checkIfExistModule(modulePath)) {
        res.sendStatus(404)
